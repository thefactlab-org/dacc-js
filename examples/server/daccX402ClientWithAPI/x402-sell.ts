...
app.get("/weather", (c) => {
  return c.json({ message: "GET: This content is a successful dacc-js x x402." });
});

app.post("/submit", (c) => {
  return c.json({ message: "POST: This content is a successful dacc-js x x402." });
});
 
// serve({ fetch: app.fetch, port: Number(port) });
export default { fetch: app.fetch, port: Number(port) };
