...
:::info
It's recommended to retrieve JWT tokens ([allowSessionTimeWalletWithJWT_V2](/functions/v2/session-wallet/create-time-jwt_V2)) from cookies that were previously stored privately and securely **using HTTP-only/In-memory cookies in web browsers** for verification and usage.
:::
:::warning
...
:::warning
Must be the same secret used in [allowSessionTimeWalletWithJWT_V2](/functions/v2/session-wallet/create-time-jwt_V2#jwtsecret).
:::
